...
Synthetic references generated by splitting the same T2T-CHM13 v2.0 (hs1) source FASTA at controlled boundaries. No bases were added, removed, or masked: every reference retains 3,117,292,070 total and non-N bases. The coordinate map records the hs1 source chromosome and interval for every fragment, enabling exact reverse coordinate translation for calls contained within a fragment. Fresh alignment, SV calling, coordinate translation, and benchmarking of the synthetic ladder remain to be completed.

## Supplementary Table S10. Round-trip coordinate-projection control

Aggregate results after projecting a native hs1 callset through a legacy reference and returning it to hs1. Outward survival is the proportion of eligible native calls retained after the hs1-to-legacy transfer; return survival is the proportion of outward survivors retained during the return transfer. The verified implementation retained PASS INS, DEL, and DUP calls with valid spans of 1–1,000,000 bp, excluded BND and INV calls, and applied a 10% maximum span-change threshold to DEL and DUP. It reports aggregate input and retained counts only; it cannot attribute losses separately to unmapped, fragmented/ambiguous, or span-filtered outcomes.
